...
class ImageSchema(Base):
    ...
    id: int = Field(..., description="Id images")


class AddTweetSchema(Base):
    tweet_data: str = Field(
        ..., description="Tweet content", min_length=10, max_length=10_000
    )
...
class UserSchemaFull(Base):
    ...
    id: int = Field(..., description="User ID")
    name: str = Field(..., description="User name")
    following: List[UserSchema] = Field(..., description="List following")
    followers: List[UserSchema] = Field(..., description="List followers")
...
class TweetSchema(Base):
    ...
    likes: List[UserSchemaLikes] = Field(..., description="List of users who liked it")
