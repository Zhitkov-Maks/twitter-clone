...
async def test_get_user_me(ac: AsyncClient):
    response = await ac.get("/api/app_users/me", headers={"api-key": "test"})
    data = response.json()
...
async def test_get_tweets_user_is_not_register(ac: AsyncClient):
    response = await ac.get("/api/tweets",
                            headers={"api-key": "notRegisterUser"})
    data = response.json()
...
async def test_get_user_by_id(ac: AsyncClient):
    response = await ac.get("/api/app_users/1", headers={"api-key": "test"})
    data = response.json()
    assert data.get("result"), data.get("user")
    assert response.status_code == 200
...
async def test_get_user_by_id_not_exists(ac: AsyncClient):
    response = await ac.get("/api/app_users/100", headers={"api-key": "test"})
    data = response.json()
...
async def test_user_add_followed(ac: AsyncClient):
    response = await ac.post("/api/app_users/4/follow",
...
async def test_user_add_followed_twice(ac: AsyncClient):
    ...
    data = response.json()
    assert not data.get("result")
    assert data.get("detail").get(
        "error_message") == "User has already been added"
    assert response.status_code == 400


async def test_user_remove_followed(ac: AsyncClient):
    response = await ac.delete("/api/app_users/4/follow",
                               headers={"api-key": "test"})
    data = response.json()
    assert data.get("result")
    assert response.status_code == 200
...
async def test_user_remove_followed_twice(ac: AsyncClient):
    ...
    assert not data.get("result")
    assert data.get("detail").get(
        "error_message") == "User has already been deleted"
    assert response.status_code == 400


async def test_user_add_followed_not_exists_user(ac: AsyncClient):
    response = await ac.post("/api/app_users/100/follow",
                             headers={"api-key": "test"})
    data = response.json()
